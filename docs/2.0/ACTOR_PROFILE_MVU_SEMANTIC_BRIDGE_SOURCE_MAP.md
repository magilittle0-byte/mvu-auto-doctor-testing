...
适用版本：`2.0.0-rc.37` 测试仓候选。rc.36 真实同一聊天十二回复及追加压力输入全部误报“人物档案无变化”，MVU/Registry/人物页始终为0人。rc.37 修复生成前票据被错误限定为原创 NPC 的上游根因：任何首次进入当前聊天、可稳定单指、尚无输入 MVU 档案的非玩家人物都绑定预生成稳定 ActorId；权威设定覆盖骰轴内容但不能跳过建档。票据提示必须在真实 prompt-ready outgoing 副本中留下精确 proof，accepted-final 才能进入票据持久化和 MVU 事务。直接相关回归 426/426 与完整自动套件 674/674 已通过；新指纹十二回合门禁尚未完成。
...
3. prompt-ready 事件先在 outgoing 副本中逐字核对/补入同一票据合同，并记录 chat/generation/serial/type/ticket digest 证明；stored chat 不改。accepted-final 适配器只读取当前最终 assistant 的精确 SourceRef，且 proof、票据批次与持久 readback 全部匹配才继续；聊天、message、swipe、generation、content、scope 或 epoch 漂移均零写入。格式先本地宽容修复，再按 ticket/ActorId 逐人物绑定和隔离。只有唯一、位置合法的显式无变化注释可成为 zero-write 成功；完整专用块和该注释都缺失时始终记录 `profile_block_missing + emptyOperations + repairable`，不能因内存票据丢失伪装成功。
