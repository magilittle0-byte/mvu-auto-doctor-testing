...
适用版本：`2.0.0-rc.39` 测试仓候选。rc.38 真实第2回合的2人回执同时暴露“可见锚点未含姓名”和 `full_adult` 六项生理全缺，最终0写入、Doctor未结算。rc.39 在专用 accepted-assistant 回执、同票 ActorId 与精确可见 `<content>` 内，把缺失/不匹配锚点本地收敛为正文中实际存在的自然姓名；姓名不在正文仍隔离。运行时 ticket prompt 与配套预设最终 V8 同时要求每个新人物写齐六段和六项生理，不适用也逐项说明。直接相关回归 201/201 与完整自动套件 677/677 已通过；当前指纹十二回合门禁尚未完成。

## 生产链

1. 生成前继续复用现有 `characterCreationTicket` 和稳定 ActorId。P4 的唯一 next-turn consumer 同时承载本回合票据、上一回合世界事件/可见后果及最多六个相关 durable-ready MVU 档案摘要；不发送全部历史档案。凡首次进入当前聊天、可稳定单指且没有输入 MVU 档案的非玩家人物都按首次出现顺序消费一票，角色卡/世界书/原著/数据库设定只覆盖冲突骰轴。票据载荷把完整 ticketId 就近绑定到同一 accepted assistant 的隐藏六段回执；确实没有上述人物且没有既有人物变化时才可交回 `<!-- 人物档案无变化 -->`。
2. 配套预设 `dist/01_主预设_人物万花筒_可调篇幅_IZUMI0814作者更新_ARGO1.3最小融合候选版.json` 与 `npcDesignTicketPrompt()` 使用同一顺序：唯一 `<content>` 内先完成空的 `<luntan></luntan>`，闭合 `</content>`，随后恰好一个隐藏人物档案/无变化收据，再继续 options/UpdateVariable/其他辅助域。V6 上下文隔离条目启用，V4 在真实 `prompt_order` 中停用，V7 首次入聊票据合同之后由 V8 `full_adult` 生理逐人物硬闸最终复核；只存在于 `prompts` 而未排序的对象不算生产接线。新人物给齐六段 V6 自然叙事档案，`full_adult` 六个 physiology field 缺一不可，年龄未明、非人或不适用也须逐项自然说明。
3. prompt-ready 事件先在 outgoing 副本中逐字核对/补入同一票据合同，并记录 chat/generation/serial/type/ticket digest 证明；stored chat 不改。accepted-final 适配器只读取当前最终 assistant 的精确 SourceRef，且 proof、票据批次与持久 readback 全部匹配才继续；聊天、message、swipe、generation、可见叙事、scope 或 epoch 漂移均零写入。人物提交后消费隐藏回执、变量回放 `UpdateVariable` 或刷新状态占位属于同一 accepted narrative 的 Doctor 自有机制刷新，不得取消并行模块；整条哈希变化时必须同时核对 accepted-content fingerprint 与本次 Doctor 精确旧/新消息改写证明，保存失败或 operation 失效会撤销/清空证明。格式先本地宽容修复；新人物锚点缺失、未命中正文或未包含姓名时，仅当同票自然姓名逐字存在于精确可见 `<content>` 才收敛为该姓名，否则隔离。数据库、世界书、选项与历史隐藏块均不能提供锚点。只有唯一、位置合法的显式无变化注释可成为 zero-write 成功；完整专用块和该注释都缺失时始终记录 `profile_block_missing + emptyOperations + repairable`，不能因内存票据丢失伪装成功。
...
The current exact paired preset is the IZUMI file named above; its candidate SHA-256 is recorded in `TESTING_CHANNEL.md` and bound into `doctorRuntimeCriticalFingerprint()`. Historical rc.27 through rc.35 observations and suites do not substitute for rc.36 current-fingerprint evidence.
