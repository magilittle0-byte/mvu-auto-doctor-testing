...
适用版本：`2.0.0-rc.15` 测试仓候选。状态：自动回归候选，真实模型、数据库、SillyTavern/Tauri 与十二回合门禁未执行。
...
本候选的完整 Node 自动套件 596/596、语法检查、JSON 解析和差异检查只是自动检查，不证明真实可用。任何后续运行代码或提示词变化都会使旧真实证据失效；正式仓与正式 main 必须等待当前源码指纹下的真实唯一验收协议。
