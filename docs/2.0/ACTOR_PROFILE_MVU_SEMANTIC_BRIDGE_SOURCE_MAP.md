...
适用版本：`2.0.0-rc.17` 测试仓候选。状态：自动回归绑定当前提交；rc.16 的一次真实首回合暴露省略块误报、P3 stale 与失败刷新丢失，rc.17 已修复但未经第二次真实生成复验；十二回合正式门禁仍未完成。
...
6. P3 与人物档案事务独立启动，冻结启动时已经 durable-ready 的人物集合。缺档人物不参与该批人物行动，结构世界、其他 ready 人物和支线仍继续；人物尝试必须由世界单批裁决。首次独立 P3 若只因同源宿主身份窗口得到 stale，P1 终态唤醒在原任务结束后对同一 accepted SourceRef 最多安全重试一次。
...
本任务唯一一次完整 Node 套件运行共 602 项，修复前为 596 通过、6 失败；6 项失败均已修复并由对应定向回归覆盖，但依照“完整套件只运行一次”的约束未再次运行整套，因此不能声称 rc.17 完整套件全绿。rc.16 唯一一次真实首回合又暴露了省略块误报、P3 stale 与刷新失败状态丢失；rc.17 已按真实证据修复并完成定向自动回归，但用户未授权第二次真实生成，所以 rc.17 没有真实复验证据。语法检查、JSON 解析、差异检查和合成 DOM 回归也不证明真实可用；正式仓与正式 main 必须等待当前源码指纹下的完整真实唯一验收协议。
