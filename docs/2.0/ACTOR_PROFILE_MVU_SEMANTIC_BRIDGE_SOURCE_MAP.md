...
适用版本：`2.0.0-rc.31` 测试仓候选。下文保留 rc.27/rc.28/rc.29/rc.30 的真实失败说明，但它们属于旧指纹，不能证明当前候选。rc.29 把 MVU durable semantic profile、A/B/C/D 四项长期优化和新的 projection recovery 接到同一候选；rc.30/rc.31 只修复真实宿主暴露的入口解析和默认开场初始化阻断并增加对应回归，不改变本页所有权或事务架构。当前完整自动套件为 664/664；十二回合真实门禁须在 rc.31 指纹下执行。运行代码或配套预设变化后须重新加载当前提交，旧真实证据立即失效。
...
## rc.29/rc.30/rc.31 A/B/C/D production map
