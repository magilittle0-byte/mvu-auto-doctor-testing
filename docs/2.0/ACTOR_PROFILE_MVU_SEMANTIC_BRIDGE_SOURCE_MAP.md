...
适用版本：`2.0.0-rc.38` 测试仓候选。rc.37 真实首回合已证明同票 ActorId、六段自然档案、`full_adult` 生理档案、MVU readback-ready 和人物额外模型调用0可以同时成立；但人物回执提交后的同消息机制刷新误使并行变量事务返回 `variable.final.stale`。rc.38 保持 chat/message/swipe/generation/scope/epoch 严格身份，以 accepted-content fingerprint 允许同一正文的 Doctor 自有回执/MVU 机制刷新；叙事变化仍零写。直接相关回归 210/210 与完整自动套件 676/676 已通过；当前指纹十二回合门禁尚未完成。
