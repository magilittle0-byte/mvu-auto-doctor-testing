...
|---|---|---|---|
| `actorLedger.actorRegistry`, identity/quarantine/migrations, `profileV6`, `pendingProfile`, and current-source `actor-registration` observation receipts | P1 | P3 may fresh-read a profile-only evolution after freezing the original scheduled ActorId set; newly ready rows never join the current schedule. A registration receipt is accepted only when it appends under the existing 120-entry normalization, binds the exact accepted target, and covers only ActorIds newly added by that P1 commit | deletion of an old ActorId; mutation of an old observation receipt; non-registration, wrong-target, or existing-actor registration; or any simultaneous action/world projection change |
