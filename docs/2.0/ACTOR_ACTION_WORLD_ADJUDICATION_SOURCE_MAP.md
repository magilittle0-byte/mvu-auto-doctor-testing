...
`revealPath` 只约束 private / observer-limited 的离屏结果。`visibility=public` 已由 `publicSummary` 和 `observableConsequence` 提供直接可观察证据，不能再强制填写未来揭示路径，否则会把正文中已经公开发生的结果误拒为无效裁决。
