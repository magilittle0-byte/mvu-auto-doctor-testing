...
本阶段修改：`index.js`、P3 测试与本 source map。未复制缝合怪的 NSFW/正文大提示词、TavernDB CRUD 或第二存储；Recall→Advance 串行 relay 是对其 tag task 和数据库 staged transaction 的最小适配。MVU、外部数据库、预设和 P4 消费代码保持独立。真实模型、host durable readback、刷新恢复和阶段四消费尚未运行验证。
