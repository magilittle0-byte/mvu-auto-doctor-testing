...
| 世界推进 | Stitches Reborn V 的 `act,scene` 推进任务、既有 `generateWorldContinuitySingleBatch` | 最小适配 | Advance 以“MVU自动医生·世界连续性医师”的身份读取 canonical recall packet 与最终正文；模型只表达 NPC 想做什么、实际结果、可观察后果和真正发生的变化。ActorRef/target/ATT、资源数组、可见性、时长和缺省形状由本地权威绑定与归一。Advance 与唯一一次定向补缺各自允许现有 fast route 做最多一次 transport-only handoff（仅 empty、transport-error、rate-limit），不对语义或校验失败切槽。只有仍缺真正结果或可观察后果时才使用一次小型定向补缺，不重发世界书/档案/正文全量。 |
...
| attempt 准备与读回 | 既有 actor-shard proposal 合同、`actorActionCandidatesFromShard`、`prepareActorActionAttempts`、`recordActorActionAttempts`、`persistActorActionAttemptsForTurn` | 最小适配 | 模型只给 `actorId/intent/candidateAction/stateChanges` 的最小语义行；本地从已冻结人物候选恢复 `interactionTargets/resourceCosts/capabilityUsed/currentGoal/waitCondition/stimulusDecisions/contact/causalChain` 等权威结构。缺失人物行不会丢人或拖垮结构世界轨，而是生成不创造事实的 `wait` 提案与 `delayed` 裁决。生产 prepare 仍传入宿主当前玩家身份索引，指向玩家的尝试只能进入前台待玩家决定。任何结果应用前仍先 durable ATT + prepared readback。 |
| 世界裁决与结果 | `parseActorShardProposal`、`prepareActorActionAttempts`、`recordActorActionAttempts`、`validateWorldAdjudicationBatch`、`settleActorActionCandidates`、`mergeActorWorldEventsIntoContinuity` | 原样复用 + 最小适配 | `stage3ValidateWorldDraftInMemory` 在持久化前把模型的自然语义结果绑定到本地 ATT/ActorRef/target，并机械补齐空资源、可见性、持续时间等技术缺省，再复用完整 player-sovereignty 与 adjudication validator；attempt 不等于 outcome。模型声称 `success/partial` 却没有任何真实状态变化时，本地降级为 `delayed`，绝不替模型发明成功结果，也不因此杀死整轮世界轨。 |
...
- **代码硬保证：** fresh ledger 中所有 due/overdue/starved NPC 都进入 `mustActorIds`，预算只限制可选探索；Recall 保留完整 ActorRef/Profile/目标/承诺/知识材料；模型遗漏人物行时本地生成安全等待收据，绝不丢人或伪造成功；ActionAttempt 与 prepared checkpoint 同 CAS/readback；Phase2、scope/target/CAS、恢复与 P4 可见投影仍 fail-closed。
- **提示词负责：** 以“世界连续性医师”身份理解人物欲望、有限知识、关系、资源和压力，只说明自然 NPC 尝试、实际结果、代价与可观察后果；绝不抄技术权威字段，也不替玩家行动、同意或感受。
- **交界：** Advance 返回最小人物语义行和世界/线程增量；代码在任何结果应用前绑定本地 ActorRef/ATT/target，规范技术缺省并运行原 authority validator。只有真正缺少语义结果时才用一次定向补缺。
...
scheduled ready actor 的模型行缺失或缺少可验证变化时，本地使用不创造事实的 wait/delayed 收据，结构世界轨继续；真正的身份权威漂移、玩家主权冲突、Recall/运输不可恢复、prepare/record 拒绝或 Phase1 readback 不符仍让 batch 零 Phase2 世界写。正文不是幕后轨的过滤器；P4 才按可见性和 convergence 投影注入。
...
- adjudication 在进入 validator 前先由本地把 `actorId/status/resultSummary/observableConsequence/appliedStateChanges` 最小语义行绑定到精确 ATT/ActorRef/target；模型无需回显技术权威。空资源、私有可见性、默认时长等可机械确定的字段由脚本补齐，声称成功却无真实变化则安全降为 delayed。完整内存 validator 返回的归一候选现在原样进入两个 Phase1 持久化分支，scheduled ATT 持久化边界还会幂等复用同一归一器；不再在第一次校验通过后丢弃归一结果、又用原始最小行触发旧严格裁决失败。只有真正缺少结果摘要或可观察后果等语义时才触发一次局部补缺；合并后仍运行同一完整 authority validator，且依旧最多 Advance+一次 repair、Phase1 前零写。
...
- Phase2 proof construction and P4/P3 verification now canonicalize each same-target attempt and receipt through the existing `normalizeActorLedger` paths before hashing. This makes an in-memory Phase2 settlement and its JSON/durable normalized representation identical (including attempt compatibility flags and receipt defaults such as `technicalFailure: false`) without removing any authority field. `stage3PersistedPackageDecision` reports a privacy-safe fixed subcode (`packet_missing`, producer/continuity/settlement target mismatch, `authority_digest_mismatch`, settlement-result mismatch, or whole-ledger mismatch) while the public package helper remains null-or-packet compatible. P4 records the subcode on its inspection result; a newer accepted target with an invalid prior proof exits through the measured finalizer and stays retryable rather than becoming permanently completed. Attempt/result and receipt lifecycle fields remain strict: changing action, status, result, `response_settled`, responseSourceRef, identity, target, ActorRef, or observable consequence changes the target authority digest. P4 exact-once remains owned by its existing continuity consumeProof; no ActorLedger receipt-consumption state machine is added here.
- Settlement proof normalization preserves every ordered result up to the existing ActorLedger capacity of 120 attempts. It rejects an over-capacity proof as a whole instead of truncating it. This keeps an eight-actor real turn byte-equivalent across Phase2 memory, durable normalization, readback, P4, and the following P3 gate; no settled actor can disappear from the proof while the count/digest still claims the complete target authority.
...
模型侧提示采用数据库/缝合怪成熟的“身份确认→职责→边界”顺序，但身份是中性的“MVU自动医生·世界连续性医师”，不引入第三方品牌或人格。提示只保留玩家边界、attempt≠outcome、自然语义和最小机器信封；完整 ready profile/worldbook 只在 Recall 材料中出现一次。技术身份、数组、默认值、枚举和持久化证明由本地 parser/policy 处理；这降低格式故障，不降低玩家主权、真实结果与持久化安全。
