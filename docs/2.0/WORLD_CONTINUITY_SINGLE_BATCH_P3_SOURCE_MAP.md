...
| attempt 准备与读回 | `actorActionCandidatesFromShard`、`prepareActorActionAttempts`、`recordActorActionAttempts`、`persistActorActionAttemptsForTurn`，以及数据库/shujuku 的本地字段类型归一化边界 | 最小适配 | Advance 返回 proposal/adjudication 草案后，本地只规范化明确结构：`from=actor.location.name`，`to=模型明确location或from`；同地点 travelTurns 固定0，不同地点把模型非负整数规范为至少1。模型若遗漏与 `stateChanges` 重复的“预期可观察迹象”，本地复用该模型已给出的首条计划变化（再退到尝试原文）补齐 ATT 预期，不把它当成实际 outcome，也不另调模型。随后仍经全部 ActorRef、档案、知识、证据、资源、能力、玩家主权与 stateChanges 门，代码在任何 outcome 应用前先 durable ATT + prepared candidate readback，世界裁决保持独立。 |
