...
- adjudication 在进入 validator 前先由本地把 `actorId/status/resultSummary/observableConsequence/appliedStateChanges` 最小语义行绑定到精确 ATT/ActorRef/target；模型无需回显技术权威。空资源、私有可见性、默认时长等可机械确定的字段由脚本补齐，声称成功却无真实变化则安全降为 delayed。只有真正缺少结果摘要或可观察后果等语义时才触发一次局部补缺；合并后仍运行同一完整 authority validator，且依旧最多 Advance+一次 repair、Phase1 前零写。
