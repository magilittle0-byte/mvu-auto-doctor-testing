...
| 医生 `acceptedFinalContinuityStartBarrier` 与 P4 `activeNextTurnConsumer.worldPackage` | 必要薄胶水 | accepted-final 只在当前 Doctor consumer 明确携带同 generation 的上一回合世界包时等待 P4 清理，再启动 P3。只有 characterCreationTicket 的 consumer 即使清理仍 pending 也不是世界依赖，P3 可与 P1 独立启动；foreign generation 或无 active consumer 同样不形成屏障。该 helper 纳入 runtime fingerprint，且不改变 P4 唯一插槽、exact-once consume 或 SourceRef 漂移零写语义。 |
