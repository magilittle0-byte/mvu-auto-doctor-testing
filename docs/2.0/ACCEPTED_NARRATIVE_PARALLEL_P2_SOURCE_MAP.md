...
| 医生公开的旧 barrier 兼容签名与 `waitForTargetSettled / runAfterTargetSettled` | 兼容保留 | 旧 register/ack/status 仅立即返回 `unmanaged / independent_modules_no_global_settlement`；不注册、不 reader、不等待、不发事件、不读取旧 receipt，也不宣称 cooperative。全局 `DownstreamBarrierProtocol`、director 与 `phase6Runtime` 根入口已退役；人物/世界仍只依赖各自真实事务收据。 |
