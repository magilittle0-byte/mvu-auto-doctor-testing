...
Doctor uses two compatible groups: route-only `identity_bootstrap` and `character_core(person/personality/history/relationshipsMotives/currentState/knowledgeCapabilitiesResources/physiology)`, with physiology targets present only for `full_adult`. The route-only probe creates no dossier content: it supplies literal displayName/row keys for local SourceRef/ActorRef locking, then character_core fills every requested missing/refresh field from the same authority/ticket snapshot. The mature batch core keeps up to six ActorRefs in one fill-group response instead of forcing one network call per person; local parsing still validates and maps every ActorRef/module cell into its own working slot, and retries only missing or invalid slots. All slots remain one atomic profile transaction. Full and full_adult therefore both need at most one normal fill-group call for an ordinary six-person-or-smaller batch after identity, rather than paying separate per-person or per-module network round-trips.
...
Identity and fill prompts now begin with the mature database/preset pattern “Identity Confirmation → role → ownership boundary”: the model is the neutral “MVU自动医生·人物档案医师”, a biography editor rather than the narrative author, database AI, MVU, or world adjudicator. This is prompt-only role framing and introduces no external brand/provider coupling. Identity receives the complete accepted narrative once as ordinary prose plus a natural registered/excluded index; coverage units remain private local proof material and are not exposed as JSON. Its normal output is a short natural Chinese list: one `新人物：正文逐字行键` line per real appearance, `身份揭示：ActorId｜新行键｜最短原句` for an exact registered reveal, or the whole response `没有新人物`; it no longer asks the model to author XML/JSON/functions/variables, opaque digests or empty units. The prompt explicitly requires the complete literal label and forbids bare vague labels such as “人物、角色、陌生人、男人、女人、他、她”. Local code mechanically normalizes these three bounded surface forms into the existing route representation, then still requires every row key and reveal evidence to bind literally to the sent coverage plan. Complete legacy flat `profile-target`, exact `<no-new/>`, and per-unit id+digest wrappers remain read-compatible. During the first-ever empty-Registry bootstrap only, an otherwise literal anchored new-person row that the model mislabeled with an invented ActorRef is mechanically normalized back to `new`; protected-name and Registry preflight still run unchanged. As soon as any ActorRef exists, an unknown id remains fail-closed so a misspelled reveal cannot duplicate or merge an actor. If the first automatic identity response produces no locally retainable route and ends in a bounded format/identity failure, the same transaction may resend that same complete input exactly once with only fixed privacy-safe failure codes and the same natural Chinese output contract. A parsed route rejected solely as `actor_profile.discovery_name_vague` is treated as non-retainable for this one resend; protected identities, Registry conflicts and ambiguous nested row keys remain one-call fail-closed. The resend never asks for legacy markup, JSON, digest or empty units; a second invalid answer terminates atomically, and an empty resend cannot erase a candidate claim from the first answer. Explicit later current-target repair remains separately bounded by the durable same-SourceRef retry receipt and carries the saved fixed failure codes into its only model call rather than issuing a generic first prompt. Its counter and fixed codes are sealed into the recovery-progress digest; a second manual click, a locked identity, any verified row, an unrelated failure code, or a different SourceRef remains fail-closed. Neither boundary is an automatic loop. The local parser binds every literal route to an explicit valid unit or to the earliest independent literal occurrence not covered by a longer returned row key. A registered-identity reveal instead binds the earliest unit containing its complete literal evidence. The complete local plan is then re-sealed as the coverage proof; this is mechanical routing over the exact text already sent, not a model-authored proof or a keyword/name scanner. Partial wrappers, unknown explicit units, target+empty mixtures, mixed natural empty+route claims, and unrecognized identity-like rows fail closed; ordinary preamble around already valid rows may be ignored. Each later fill prompt contains independent module Notes, exact ActorRef x field targets, fresh values only for those fields, bounded row-local narrative evidence, bounded relevant authority and the same normalized ticket. It does not resend the complete narrative, the 42K authority envelope, unrelated dossiers, commit proofs, or persistence history. Each actor row and authority projection appears once per compatible group; a separate requestedModules map lists actor-module cells. Compact JSON in targetRows is hidden routing metadata only; authored values remain natural Chinese. The response uses only lightweight target/module/physiology-field routing tags, never user-visible fixed headings. The parser tolerates fences, surrounding prose, Chinese quotes and explicit tag/bracket/label drift, but never guesses a slot from Chinese semantic keywords. Duplicate, unexpected, or missing targets/modules/physiology fields fail closed. The old single-dossier/seven-heading parser is load-time compatibility only and is not a production generation fallback.

Discovery ticket order is derived from literal accepted-narrative positions, not model row order. `parseActorProfileModuleGroupOutput` first binds each identity row to a locally verified coverage unit and its cumulative narrative start. Legacy wrappers still verify their echoed digest; compact flat routes use the already-sent local plan and never accept a model-supplied digest. `actorProfileDiscoverySourceOrder` enumerates literal occurrences only inside that unit; when one valid row key is a prefix/substring of another, it rejects occurrences covered by the longer candidate key and selects the first independent occurrence. The verified `{coverageUnitId, sourceUnitOffset, sourceOffset, sourceOrdinal}` then travels through the existing recovery receipt, final `candidateRef`, `acceptedModelProfileDiscoveryFacts`, Registry promotion and ticket binding; downstream code verifies the exact narrative slice and unit span instead of running a new global `indexOf`. Thus a shorter key mentioned independently later cannot steal the longer key's first offset, while a key that never has an independent literal occurrence remains `discovery_source_offset_ambiguous` and cannot be promoted. This is local deterministic routing only: it neither drops a candidate nor relaxes Registry, coverage, anchor, conflict, or identity preflight.
       ...
       └─ runActorProfileTarget
            ...
            └─ completeActorProfilesForTurn
                 └─ completeActorProfileBatchTransaction
                      ├─ identity_bootstrap 使用同一完整 coverage plan；正常响应是自然中文“新人物/身份揭示”逐行清单或全响应“没有新人物”，本地机械归一、逐字绑定后重建 proof；旧 route/`<no-new/>` 只读兼容；首答零可保留行且为固定格式/身份失败时自动定向重发至多一次，重发仍只要求自然中文清单，第二次无效即原子失败
...
同一 accepted source 的 P1/P3 并发与身份格式收口
  └─ 恢复收据已耐久回读、且同目标已无运行中的 P3 owner 时，enqueueActorProfiles
     允许两种一次性恢复：已经验证字段的纯 actor_profile.target_stale 直接复用既有
     字段零模型重放；零字段、零行、身份未锁定且唯一格式故障为
     actor_candidate.identity_missing_or_short（可伴随同轮 target_stale）时，使用收据内
     唯一 identity retry 额度重发一次身份发现。两者都在原 owner 内复用同一个
     runActorProfileTarget(force=true, allowIdentityRetry=true) 与既有 pending/final
     CAS/readback；不递归入队、不建立新 store、不无限循环。目标/scope 已变、P3 尚在
     运行、恢复材料缺失、身份已锁定或出现其他 failure code 时仍保留 not_completed。
...
| TavernDB / npc_tracker 非空唯一人物行键 | **T：**保留非空、唯一、同 Registry 冲突拒绝；**A：**正文中的完整姓名、编号、职业或带限定的描述性称谓可作为逐字 display label；裸“人物、角色、陌生人、男人、女人、他、她”不作为稳定行键。若同一 coverage unit 内有唯一明确引号或“名为/称作”证据可机械恢复，否则固定报含糊标签并保持零写。|
...
- 0 人：合格自然正文执行 route-only identity bootstrap。只有整个响应是明确的自然中文“没有新人物”同义终态或旧版精确 `<no-new/>`，且本地确认本次完整 coverage plan 已发送、current-source incomplete/missing/unexpected/unresolved/quarantine/schema/ticket/commit/readback/maintenance failure 全为空，才从本地 plan 重建并密封 no-candidates proof。自由解释、人物清单与 empty 混用、部分 legacy wrapper 或未知 unit 均不能证明零人物。首答零可保留行且命中固定格式/身份失败时可自动定向重发一次；首答若已提出但未通过的人物候选，重发的空答不得抹除该证据；第二次无效即原子失败。模块 off、机制-only、空正文才是 0 调用。
...
- 正常 full 与 `full_adult` 批次是一次 identity、一次同行 character_core；只有首答零可保留行的固定 identity 失败或 character_core 缺项才各自允许一次定向重发。已 ready module 不重调，先前成功 field 只缓存在本次 transaction 内存；运输失败只由 route/failover 处理，不伪装成语义成功。
