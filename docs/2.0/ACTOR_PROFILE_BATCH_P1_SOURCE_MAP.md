       ...
       └─ runActorProfileTarget
            ...
            └─ completeActorProfilesForTurn
                 └─ completeActorProfileBatchTransaction
                      ├─ identity_bootstrap 只读一次完整 coverage plan；正常响应是自然中文“新人物/身份揭示”逐行清单或全响应“没有新人物”，本地机械归一、逐字绑定后重建 proof；旧 route/`<no-new/>` 只读兼容，不回显 digest/空 unit，不因格式错误重发完整正文
...
- 0 人：合格自然正文只执行 route-only identity bootstrap。只有整个响应是明确的自然中文“没有新人物”同义终态或旧版精确 `<no-new/>`，且本地确认本次完整 coverage plan 已发送、current-source incomplete/missing/unexpected/unresolved/quarantine/schema/ticket/commit/readback/maintenance failure 全为空，才从本地 plan 重建并密封 no-candidates proof。自由解释、人物清单与 empty 混用、部分 legacy wrapper 或未知 unit 均不能证明零人物。首次响应若已有候选或 identity/schema failure，后续空答不得抹除；身份不因格式错误二次发送完整正文。模块 off、机制-only、空正文才是 0 调用。
