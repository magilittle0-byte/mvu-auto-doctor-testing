...
当模型把新人物路由成裸泛称时，Doctor 只在同一 accepted-narrative coverage unit 内发现唯一的明确引号或“名为/称作”证据时机械替换为该原文标签；没有明确证据或存在多个标签仍保持 `actor_profile.discovery_name_vague`、零写，并交给修复中心处理。该适配不从形容词、职业或自由叙述猜名。
