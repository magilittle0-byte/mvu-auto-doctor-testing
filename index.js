        ...
        ...(actorShardCandidates
            ? [
                ...
                worldCreatesAttempts
                    ? '本轮已调度人物必须各输出一条actionProposals和一条同actorId的actionAdjudications。提案只写NPC自己的尝试；必须说明预期代价、耗时、风险，以及“若尝试发生后可观察或验证到什么”，后者只是预期迹象，不是已经发生的世界结果。location为空或等于人物当前地点时travelTurns必须为0，明确前往不同地点时必须为>=1的整数。本地会先用ActorRef、档案、知识、地点、资源、能力、因果和玩家主权进行验证并持久化attempt，才会应用你给出的世界裁决。此分支尚未存在ATT：裁决草案只能给actorId，禁止编造attemptId、actorRef或target。不得替玩家行动。'
